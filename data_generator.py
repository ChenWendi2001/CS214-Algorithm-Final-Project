...
def generate_DAG(job_id):
    ...
    for i in range(n):
        if i == 0 or i == n-1 :
            continue
        task_list.append({"name":job_id+"_task"+str(i),"resource":[],"time" :1+random.random()*3})
        local_resource = []
        ...
        for j in range(num_of_local_resource):
            ...
            if resource[r] not in local_resource:
                ...
                task_list[i-1]["resource"].append({"name":resource[r],"size" : random.randint(50,300)})
        

    for e in edges:
        if e[0] == 0 or e[1] == n-1:
            continue
        constraint.append([task_list[e[0]-1]["name"],task_list[e[1]-1]["name"]])
        task_list[e[1]-1]["resource"].append({"name":task_list[e[0]-1]["name"],"size" : random.randint(50,300)})
...
def generate_DC(data):
    DC = {}
    DC_slot_size = [2,1,3,2,1,5,2,2,2,1,2,4,4] # same as the toy data
    DC["DC"] = []
    for i in range(13):
        DC["DC"].append({"name":"DC"+str(i+1),"size":DC_slot_size[i],"data":[]})
    for d in data:
...
def generate_job_list(n):
    ...
    constraint_json = json.dumps(constraint, sort_keys=True, indent=4, separators=(',', ': '))
