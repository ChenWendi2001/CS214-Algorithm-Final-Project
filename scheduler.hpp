...
public:
    ...
    void sumbitTasks(unordered_set<string> tasks)
    {
        for(string each_task:tasks){
            ...
            slot=graph->slots.begin();slot!=graph->slots.end();++slot){
                if((slot->second).first!=0){//capacity>0
                    Arrange this_method;
    ...
    vector<Arrange> getScheduled()
    {
        ...
        while(!mapping.empty()){
            ...
            mapping.pop();
            if(ready_queue.find(assignment.second.first)
            !=ready_queue.end()){//tasks in ready_queue
